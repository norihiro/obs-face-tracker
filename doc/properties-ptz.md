...
Currently `None` (just output to the log file) and `VISCA over IP` is provided.
Another option `VISCA over serial port` will be enabled if you build from the source with option `WITH_PTZ_SERIAL=ON`.
